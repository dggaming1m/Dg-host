...
if not TOKEN:
    raise Exception("❌ BOT_TOKEN not found in environment. Please set it in .env or hosting environment.") # Replace with your actual token
...
def handle_zip_file(downloaded_file_content, file_name_zip, message):
    ...
    try:
        ...
        # Move all extracted files to permanent user folder
        logger.info(f"Moving extracted files to permanent user folder: {user_folder}")
        for item in os.listdir(temp_dir):
            ...
            try:
                ...
                logger.error(f"❌ Error moving {item} to {dst}: {move_error}")

        # Confirm file exists in permanent folder before running
        main_script_path = os.path.join(user_folder, main_script_name)
        if not os.path.exists(main_script_path):
            bot.reply_to(message, f"❌ Error: Main script not found after move: {main_script_path}")
            return
